...
void GameManager::createHumanoid(int nbreVampire, int nbrHuman){
    ...
    //creation du Buffy
    xPosition=rand()%_hauteurGrille;
    yPosition=rand()%_largeurGrille;
    _field->addHumanoid(new Buffy(xPosition, yPosition));

    for(int i=0; i< nbrHuman; i++){
        //creation de Humain
        xPosition=rand()%_hauteurGrille;
        yPosition=rand()%_largeurGrille;
        _field->addHumanoid(new Humain(xPosition, yPosition));
    }
    ...
    for(int j=0; j< nbreVampire; j++){
        ...
        xPosition=rand()%_hauteurGrille;
        yPosition=rand()%_largeurGrille;
        _field->addHumanoid(new Vampire(xPosition, yPosition));
    }
}

void GameManager::loadListHumanoidGrid(){
    for (_List_iterator<Humanoid *> it = _field->getListHumanoid().begin(); it != _field->getListHumanoid().end(); ++it ){
